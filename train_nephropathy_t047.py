#!/usr/bin/env python
"""
Train diabetic nephropathy prediction model using ONLY T047 (Disease or Syndrome) conditions.

This is an experimental variant that filters out social determinants of health (SDOH)
and other non-disease conditions to focus on clinical disease patterns.

Comparison with train_nephropathy.py:
- Original: Uses ALL condition semantic types (T047, T033, T054, T055, etc.)
- This version: Uses ONLY T047 (Disease or Syndrome) conditions

This helps answer: "What clinical disease patterns predict nephropathy,
independent of social determinants?"

Semantic types filtered OUT:
- T033 (Finding): unemployment, abuse, limited social contact
- T054 (Social Behavior): social isolation
- T055 (Individual Behavior): unhealthy alcohol drinking

Semantic type KEPT:
- T047 (Disease or Syndrome): diabetes, neuropathy, CKD, etc.

Medications, procedures, and observations are kept unchanged.
"""
...
def get_patient_events_t047(client: PatientGraphClient, patient_ids: list) -> pd.DataFrame:
    ...
    - Conditions: ONLY T047 (Disease or Syndrome) - excludes SDOH
    - Medications: All (unchanged)
    - Procedures: All (unchanged)
    - Observations: All (unchanged)
    """
    patient_values = " ".join([f'"{pid}"' for pid in patient_ids])
...
def main():
    print("=" * 70)
    print("DIABETIC NEPHROPATHY PREDICTION - T047 ONLY (Disease/Syndrome)")
    print("=" * 70)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("\nNOTE: This model uses ONLY T047 conditions (excludes SDOH)")
    ...
    print("Condition filter: T047 (Disease or Syndrome) ONLY")
    ...
    # =========================================================================
    # Step 4: Extract events (T047 CONDITIONS ONLY)
    # =========================================================================
    print("\n--- Step 4: Extracting patient events (T047 conditions only) ---")
    ...
    with PatientGraphClient() as client:
        for i in range(0, len(all_patient_ids), batch_size_extract):
            ...
            try:
                events_df = get_patient_events_t047(client, batch_ids)
    ...
    # Save model (different filename for comparison)
    torch.save({
        ...
        'condition_filter': 'T047_only'
    }, model_path / "nephropathy_t047_model.pt")

    # Save results JSON
    results = {
        'timestamp': datetime.now().isoformat(),
        'task': 'diabetic_nephropathy_prediction',
        'condition_filter': 'T047_only (Disease or Syndrome)',
    ...
    with open(model_path / 'nephropathy_t047_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"Model saved to: {model_path / 'nephropathy_t047_model.pt'}")
    print(f"Results saved to: {model_path / 'nephropathy_t047_results.json'}")

    print("\n" + "=" * 70)
    print("Training complete! (T047 conditions only)")
    print(f"Test AUROC: {test_auc:.4f}")
    print(f"Test AUPRC: {test_auprc:.4f}")
    print("=" * 70)
