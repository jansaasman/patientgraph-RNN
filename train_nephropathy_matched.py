#!/usr/bin/env python
"""
Train diabetic nephropathy prediction model with MATCHED case-control design.

Key differences from original:
1. Cases: ALL patients with kidney disease (not just diabetics)
2. Controls: Matched on demographics (age, gender), must have prediabetes/diabetes
3. Cases: ALL events up to kidney diagnosis (no lookback limit)
4. Controls: 5-year lookback window before last event
5. No prediction gap - use all events up to kidney diagnosis

This is a proper epidemiological case-control design.
"""
...
def get_kidney_disease_patients(client: PatientGraphClient) -> pd.DataFrame:
    """
    Get ALL patients with kidney disease and their demographics.

    Returns DataFrame with: patientId, kidneyDate, birthDate, gender
    """
    query = f"""{PREFIXES}
    SELECT ?patientId
...
def get_prediabetic_patients(client: PatientGraphClient) -> pd.DataFrame:
    """
    Get patients with prediabetes or diabetes (potential controls).

    Returns DataFrame with: patientId, diabetesDate, birthDate, gender
    """
    query = f"""{PREFIXES}
    SELECT ?patientId
    ...
    WHERE {{
        ...
        # Match diabetes/prediabetes
        FILTER(
            CONTAINS(LCASE(?label), "diabetes") ||
            CONTAINS(LCASE(?label), "diabetic") ||
            CONTAINS(LCASE(?label), "prediabetes")
        )
    }}
    GROUP BY ?patientId
    ORDER BY ?patientId
...
def get_patient_events_t047(client: PatientGraphClient, patient_ids: list) -> pd.DataFrame:
    ...
    WHERE {{
        ...
        {{
            # CONDITIONS: Filter to T047 (Disease or Syndrome) only
...
def match_controls(cases_df: pd.DataFrame, potential_controls_df: pd.DataFrame,
                   ratio: int = 2) -> pd.DataFrame:
    """
    Match controls to cases based on demographics.

    For each case, find up to `ratio` controls with:
    - Same gender
    - Age within 5 years

    Args:
        cases_df: DataFrame with case patients (patientId, birthDate, gender)
        potential_controls_df: DataFrame with potential controls
        ratio: Number of controls per case

    Returns:
        DataFrame of matched controls
    """
    # Calculate age at reference date (use a fixed reference for consistency)
    ...
    for _, case in cases_df.iterrows():
        ...
        if pd.notna(case_age):
            ...
            candidates = candidates[candidates['age_diff'] <= 5]
            candidates = candidates.sort_values('age_diff')

        # Take up to `ratio` controls
...
def main():
    print("=" * 70)
    print("DIABETIC NEPHROPATHY PREDICTION - MATCHED CASE-CONTROL DESIGN")
    print("=" * 70)
    ...
    print(f"  Cases: events up to (kidney diagnosis - {prediction_gap_days} days)")
    ...
    print(f"  Controls: prediabetic/diabetic patients without kidney disease")
    print(f"  Matching: age (±5 years), gender")

    # =========================================================================
    # Step 1: Get kidney disease patients (CASES)
    # =========================================================================
    print("\n--- Step 1: Getting kidney disease patients (cases) ---")

    with PatientGraphClient() as client:
        cases_df = get_kidney_disease_patients(client)

    cases_df['kidneyDate'] = pd.to_datetime(cases_df['kidneyDate'])
    print(f"Total kidney disease patients (cases): {len(cases_df)}")

    # Calculate cutoff for cases: kidney diagnosis date - prediction gap
    # This creates a 6-month prediction horizon
    cases_df['cutoffDate'] = cases_df['kidneyDate'] - pd.Timedelta(days=prediction_gap_days)
    cases_df['windowStart'] = pd.Timestamp('1900-01-01', tz='UTC')  # No limit - use all history
    cases_df['outcome'] = 1

    # =========================================================================
    # Step 2: Get prediabetic patients (potential controls)
    # =========================================================================
    print("\n--- Step 2: Getting prediabetic/diabetic patients (potential controls) ---")

    with PatientGraphClient() as client:
        prediab_df = get_prediabetic_patients(client)

    print(f"Total prediabetic/diabetic patients: {len(prediab_df)}")

    # Exclude patients who have kidney disease
    case_ids = set(cases_df['patientId'])
    potential_controls = prediab_df[~prediab_df['patientId'].isin(case_ids)].copy()
    print(f"Potential controls (prediabetic without kidney disease): {len(potential_controls)}")
    ...
    # Get last event date for controls to set their cutoff
    ...
    # For cases: events in [kidneyDate - 5 years, kidneyDate)
    ...
    # For controls: events in [lastEvent - 5 years, lastEvent)
    # First, get last event date per control
    ...
    # Save a summary with samples
    summary_path = output_dir / "matched_events_summary.txt"
    with open(summary_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("MATCHED CASE-CONTROL TRAINING DATA SUMMARY\n")
        f.write("=" * 70 + "\n\n")

        f.write(f"Total cases (kidney disease): {len(cases_df)}\n")
        f.write(f"Total matched controls: {len(matched_controls)}\n")
        f.write(f"Total events: {len(all_events_df):,}\n\n")

        f.write("Event type breakdown:\n")
        for etype, count in all_events_df['eventType'].value_counts().items():
            f.write(f"  {etype}: {count:,}\n")

        f.write("\n" + "-" * 70 + "\n")
        f.write("SAMPLE CASE EVENTS (first 50):\n")
        f.write("-" * 70 + "\n")
        sample_cases = case_events_df.head(50)
        for _, row in sample_cases.iterrows():
            label = row.get('eventLabel', '') or row.get('eventCode', 'N/A')
            f.write(f"  {row['patientId'][:20]:20s} | {row['eventDateTime']} | {row['eventType']:12s} | {str(label)[:40]}\n")

        f.write("\n" + "-" * 70 + "\n")
        f.write("SAMPLE CONTROL EVENTS (first 50):\n")
        f.write("-" * 70 + "\n")
        sample_controls = control_events_df.head(50)
        for _, row in sample_controls.iterrows():
            label = row.get('eventLabel', '') or row.get('eventCode', 'N/A')
            f.write(f"  {row['patientId'][:20]:20s} | {row['eventDateTime']} | {row['eventType']:12s} | {str(label)[:40]}\n")

        f.write("\n" + "-" * 70 + "\n")
        f.write("UNIQUE EVENT CODES (top 50 by frequency):\n")
        f.write("-" * 70 + "\n")
        code_counts = all_events_df.groupby(['eventType', 'eventCode', 'eventLabel']).size().reset_index(name='count')
        code_counts = code_counts.sort_values('count', ascending=False).head(50)
        for _, row in code_counts.iterrows():
            label = row.get('eventLabel', '') or 'N/A'
            f.write(f"  {row['count']:6d} | {row['eventType']:12s} | {row['eventCode']} | {str(label)[:40]}\n")
    ...
    }, output_dir / "nephropathy_matched_model.pt")

    # Save results JSON
    results = {
        'timestamp': datetime.now().isoformat(),
        'task': 'diabetic_nephropathy_prediction',
        'design': 'matched_case_control_6mo_gap',
    ...
    with open(output_dir / 'nephropathy_matched_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"Model saved to: {output_dir / 'nephropathy_matched_model.pt'}")
    print(f"Results saved to: {output_dir / 'nephropathy_matched_results.json'}")
