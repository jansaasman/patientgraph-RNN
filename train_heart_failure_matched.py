#!/usr/bin/env python
"""
Train heart failure prediction model with MATCHED case-control design.

Design:
1. Cases: Patients with heart failure / congestive heart failure
2. Controls: Patients with cardiovascular risk factors but no HF
3. Cases: ALL events up to HF diagnosis (no lookback limit)
4. Controls: 5-year lookback window before last event
5. Matched on demographics (age, gender)

Note: 280 HF cases exist, using 3:1 control ratio.
"""
...
def get_heart_failure_patients(client: PatientGraphClient) -> pd.DataFrame:
    """Get all patients with heart failure and their demographics."""
    query = f"""{PREFIXES}
    SELECT ?patientId
           (MIN(?diagDate) AS ?hfDate)
    ...
    WHERE {{
        ...
        FILTER(
            CONTAINS(LCASE(?label), "heart failure") ||
            CONTAINS(LCASE(?label), "congestive heart")
        )
    }}
    GROUP BY ?patientId
    ORDER BY ?patientId
...
def get_cardiovascular_risk_patients(client: PatientGraphClient) -> pd.DataFrame:
    ...
    Excludes patients who have heart failure.
    """
    query = f"""{PREFIXES}
    SELECT ?patientId
    ...
    WHERE {{
        ...
        FILTER(
            ...
            CONTAINS(LCASE(?label), "ischemic heart") ||
            CONTAINS(LCASE(?label), "atrial fibrillation")
        )

        # Exclude HF patients
        FILTER NOT EXISTS {{
            ?patient ns28:patientCondition ?hfCond .
            ?hfCond ns28:code ?hfCode .
            ?hfCode skos:prefLabel ?hfLabel .
            FILTER(
                CONTAINS(LCASE(?hfLabel), "heart failure") ||
                CONTAINS(LCASE(?hfLabel), "congestive heart")
            )
        }}
    }}
    GROUP BY ?patientId
...
def match_controls(cases_df: pd.DataFrame, potential_controls_df: pd.DataFrame,
    ...
    for _, case in cases_df.iterrows():
        ...
        if pd.notna(case_age):
            ...
            candidates = candidates[candidates['age_diff'] <= 10]
...
def main():
    print("=" * 70)
    print("HEART FAILURE PREDICTION - MATCHED CASE-CONTROL DESIGN")
    print("=" * 70)
    ...
    control_ratio = 3  # 3 controls per case
    batch_size = 32
    epochs = 40
    learning_rate = 0.001
    ...
    print(f"  Cases: events up to (HF diagnosis - {prediction_gap_days} days)")
    ...
    print(f"  Controls: cardiovascular risk patients without HF")
    print(f"  Matching: age (±10 years), gender")

    # =========================================================================
    # Step 1: Get HF patients (CASES)
    # =========================================================================
    print("\n--- Step 1: Getting heart failure patients (cases) ---")

    with PatientGraphClient() as client:
        cases_df = get_heart_failure_patients(client)

    cases_df['hfDate'] = pd.to_datetime(cases_df['hfDate'])
    print(f"Total HF patients (cases): {len(cases_df)}")

    # Use ALL events up to (HF diagnosis - prediction gap)
    cases_df['cutoffDate'] = cases_df['hfDate'] - pd.Timedelta(days=prediction_gap_days)
    ...
    # For cases: all events up to HF diagnosis
    ...
    events_output_path = output_dir / "heart_failure_training_events.csv"
    ...
    print(classification_report(test_labels, test_preds_binary,
                                target_names=['No HF', 'Heart Failure'],
                                zero_division=0))
    ...
    }, output_dir / "heart_failure_matched_model.pt")

    results = {
        'timestamp': datetime.now().isoformat(),
        'task': 'heart_failure_prediction',
    ...
    with open(output_dir / 'heart_failure_matched_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"Model saved to: {output_dir / 'heart_failure_matched_model.pt'}")
    print(f"Results saved to: {output_dir / 'heart_failure_matched_results.json'}")
